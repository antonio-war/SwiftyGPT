...
extension SwiftyGPT {
    ...
    public func image(prompt: String, @SwiftyRanged(1...10) choices: Int, size: SwiftyGPTImageSize? = nil, user: String? = nil, completion: @escaping (Result<[NSImage], Error>) -> ()) {
        ...
        SwiftyHTTP.request(SwiftyGPTRouter.image(apiKey, request)) { result in
            ...
            case .success(let response):
                if response.statusCode == 200 {
                    ...
                    completion(.success(body.data.compactMap({ Data(base64Encoded: $0.b64) }).compactMap({ NSImage(data: $0) })))
                } else {
                    guard let error = try? JSONDecoder().decode(SwiftyGPTError.self, from: response.body) else {
                        completion(.failure(URLError(.badServerResponse)))
                        return
    ...
    public func image(prompt: String, @SwiftyRanged(1...10) choices: Int, size: SwiftyGPTImageSize? = nil, user: String? = nil) async -> Result<[NSImage], Error> {
    ...
    public func image(prompt: String, size: SwiftyGPTImageSize? = nil, user: String? = nil, completion: @escaping (Result<NSImage, Error>) -> ()) {
        image(prompt: prompt, choices: 1, size: size, user: user) { result in
            switch result {
            case .success(let images):
                if let image = images.first {
    ...
    public func image(prompt: String, size: SwiftyGPTImageSize? = nil, user: String? = nil) async -> Result<NSImage, Error> {
        return await withCheckedContinuation { continuation in
            image(prompt: prompt, size: size, user: user) { result in
                continuation.resume(returning: result)
            }
